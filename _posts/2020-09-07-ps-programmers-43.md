...
### [ 영어 끝말잇기 ](https://programmers.co.kr/learn/courses/30/lessons/12981) - lv.2
...
vector<int> solution(int n, vector<string> words) {
    vector<int> answer = {};
    map<string, int> recode;
    recode.insert({words[0], 1});
    
    int cycle = 1;
    int count = 2;
    for (auto i = 1 ; i < words.size() ; ++i) {
        if (words[i-1].back() == words[i].front() &&
            recode.find(words[i]) == recode.end()) {
            recode[words[i]] = count;
            ++count;
        } else {
            break;
        }
